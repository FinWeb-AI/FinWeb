...
import os, uuid, logging, configparser, requests, time, sys, types, ctypes
...
fake_np.array = lambda *a, **k: []
fake_np.__version__ = "0.0"
...
def verify_recaptcha(tok:str)->bool:
    ...
    try:
        r = requests.post("https://www.google.com/recaptcha/api/siteverify", data={"secret":s,"response":tok}, timeout=4)
...
def register():
    if request.method=="POST":
        ...
        if User.query.filter_by(username=u).first():
            ...
            db.session.add(User(username=u,email=e,password=generate_password_hash(p),confirm_code=c,confirm_expire=exp))
            db.session.commit()
            link=url_for("confirm",_external=True)
            mail.send(Message("FinWeb 電子郵件驗證",recipients=[e],body=f"驗證碼：{c}\n請於一小時內前往 {link} 輸入"))
            flash("註冊成功！驗證碼已寄至信箱","success"); return redirect(url_for("confirm"))
...
def confirm():
    if request.method=="POST":
        ...
        if not u:
            ...
            flash("已完成驗證，請直接登入。","info"); return redirect(url_for("login"))
        elif u.confirm_code!=cd:
            flash("驗證碼錯誤","danger")
        elif now>u.confirm_expire:
            flash("驗證碼已過期，請重新註冊。","warning")
        else:
            u.confirmed=True; u.confirm_code=None; u.confirm_expire=None; db.session.commit()
            flash("電子郵件驗證成功！請登入。","success"); return redirect(url_for("login"))
...
def login():
    if request.method=="POST":
        ...
        if not u or not check_password_hash(u.password,p):
            ...
            login_user(u); flash(f"歡迎回來，{u.username}","success"); return redirect(url_for("index"))
...
def get_stock(sym:str): return {"symbol":sym,"price":0}
def get_crypto(cid:str): return {"id":cid,"price":0}

@app.route("/api/stock")
def api_stock(): return jsonify(get_stock(request.args.get("symbol","AAPL")))
@app.route("/api/crypto")
def api_crypto(): return jsonify(get_crypto(request.args.get("id","bitcoin")))
